...
/*****************************局部函数*****************************/
//#define two_armid    0 //2关节GO电机id
#define three_armid  1 //3关节GO电机id
#define tail_armid   0 //尾巴GO电机id

#define stop_mode      0 //GO电机停止
#define foc_mode       1 //GO电机foc控制
#define calibrate_mode 2 //GO电机编码器整定
...
typedef struct
{
    ...
    // 待发送的各项数据
    unsigned short id;                  //电机ID，15表示向全部电机广播，此时没有数据反馈
    unsigned short mode;                // 0:停止, 1:foc控制, 2:电机编码器标定
    //实际给FOC的指令力矩为：
    // K_P*delta_Pos + K_W*delta_W + T
...
typedef struct
{
    ...
    int correct;                        //接收数据是否完整（1完整，-1不完整）
    //解读得出的电机数据
    unsigned char motor_id;             //电机ID
    unsigned char mode;                 // 0:空闲, 1:foc控制, 2:电机编码器标定
    int Temp;                           //温度
