...
#include "string.h"
/*****************************局部函数*****************************/
uint8_t Saber_RxData[60];
uint8_t Text_RxData[60];
...
void SaberInertia_USART6_RxCpltCallBack(void)
{
	...
	volatile uint32_t soft_set_zero;
	//判断是否为空闲中断
...
//		/*软件清零*/
//		soft_set_zero = HAL_UART_Receive(&huart6, Saber_RxData, 60, 10);
		/*关闭DMA传输*/
		...
		if((DMA_USART6_RxBuf[0] == 'A') && (DMA_USART6_RxBuf[1] == 'x') && (DMA_USART6_RxBuf[Saber_RxLen-1] == 'm'))
		{
			...
			Chassis_Machine.Beta = CapeEuler_RxData.yaw;
		}
		memcpy(Text_RxData, DMA_USART6_RxBuf, Saber_RxLen);
		/*缓存区数据清零*/
