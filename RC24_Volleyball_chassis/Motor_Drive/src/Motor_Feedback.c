...
//				average_spd = Move_Average_Filter((float *)(M3508_State_Data[id].spd));
			} break;
/**********************************机械臂**********************************/
			case 0x100:
			{
...
//          a1_time_last = HAL_GetTick();//计算中断轮询时间
				...
				{
					GO_TwoArm_RecvData.correct = 1;
				  memcpy(GO_TwoArm_RecvData.motor_recv_data.head, GOM8010_Rx, 2); //将GOM8010_Rx的前两个字节复制到head数组中
				  GO_TwoArm_RecvData.motor_recv_data.mode.id = 0;
				  GO_TwoArm_RecvData.motor_recv_data.mode.status = (GOM8010_Rx[2] >> 4) & 0x07;  // 将 GOM8010_Rx[2] 中的高 3 位赋值给 status
				  memcpy(&GO_TwoArm_RecvData.motor_recv_data.fbk, GOM8010_Rx + 3, sizeof(RIS_Fbk_t)); //假设RIS_Fbk_t类型的数据占用了11个字节，将GOM8010_Rx的第四个字节开始的11个字节复制到fbk中
				  GO_TwoArm_RecvData.motor_recv_data.CRC16 = *((uint16_t*)(GOM8010_Rx + 14)); // 假设CRC16占用了最后两个字节，将GOM8010_Rx的倒数第二、倒数第一个字节构成的uint16_t数据赋值给CRC16
				  extract_data(&GO_TwoArm_RecvData);//数据解算
				} break;
